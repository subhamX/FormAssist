...
class AadhaarOCR extends StatefulWidget {
  @override
  _AadhaarOCRState createState() => _AadhaarOCRState();
}

class _AadhaarOCRState extends State<AadhaarOCR> {
  File pickedImage;
  ...
  String dob = "NO_VAL", cardNum = "NO_VAL", gender = "NO_VAL";
  ...
  Future<void> showPopUp(int id) async {
    ...
    if (id == 1) {
      ...
      fieldPattern = genderPattern;
      fieldName = "Gender";

      myController.text = gender;
    } else if (id == 3) {
              ...
              onPressed: () async {
                if (_key.currentState.validate()) {
                  ...
                  if (result == 'error') {
                    ...
                    Flushbar(
                      message: "$fieldName Updation Successful!",
  ...
  final cardNumPattern = RegExp(r'^\d{4}\s\d{4}\s\d{4}');
  final genderPattern = RegExp(r'male$|female$', caseSensitive: false);
  ...
  Future readText() async {
    ...
    for (TextBlock block in readText.blocks) {
      for (TextLine line in block.lines) {
        newList.add(line);
        if (dobPattern.hasMatch(line.text)) {
        ...
        } else if (genderPattern.hasMatch(line.text)) {
          gender = genderPattern.stringMatch(line.text);
      ...
      child: SingleChildScrollView(
        child: Column(
          children: <Widget>[
            ...
            ListTile(
              title: Text("Aadhaar Card Number"),
